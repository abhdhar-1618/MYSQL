...
FROM employees e
JOIN (SELECT emp_no, salary FROM salaries WHERE salary > 60000) s
ON e.emp_no = s.emp_no;
...
SELECT e.first_name, e.last_name, ROUND(AVG(s.salary),2) AS salary
FROM employees e
JOIN salaries s ON e.emp_no = s.emp_no
WHERE s.salary > 60000
...
FROM employees e
JOIN salaries s ON e.emp_no = s.emp_no
WHERE e.hire_date >= '2000-01-01' AND s.salary <=60000
...
-- list of employees with unique first names
...
-- find department wise gender count
...
SELECT
    'Male' AS gender,
    SUM(CASE WHEN d.dept_name = 'Sales' THEN 1 ELSE 0 END) AS Sales,
    SUM(CASE WHEN d.dept_name = 'Research' THEN 1 ELSE 0 END) AS Research,
    SUM(CASE WHEN d.dept_name = 'Quality Management' THEN 1 ELSE 0 END) AS `Quality Management`,
    SUM(CASE WHEN d.dept_name = 'Production' THEN 1 ELSE 0 END) AS Production,
    SUM(CASE WHEN d.dept_name = 'Marketing' THEN 1 ELSE 0 END) AS Marketing,
    SUM(CASE WHEN d.dept_name = 'Human Resources' THEN 1 ELSE 0 END) AS `Human Resources`,
    SUM(CASE WHEN d.dept_name = 'Finance' THEN 1 ELSE 0 END) AS Finance,
    SUM(CASE WHEN d.dept_name = 'Development' THEN 1 ELSE 0 END) AS Development,
    SUM(CASE WHEN d.dept_name = 'Customer Service' THEN 1 ELSE 0 END) AS `Customer Service`
FROM departments d
JOIN dept_emp de ON d.dept_no = de.dept_no
JOIN employees e ON de.emp_no = e.emp_no
WHERE e.gender = 'M'
